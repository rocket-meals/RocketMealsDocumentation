...
In the `C vector add example <https://github.com/rocket-meals/RocketMealsDocumentation/blob/master/examples/c/vector_add.py>`__ we are using the ``omp_get_wtime()`` function from OpenMP to measure time on the CPU.
